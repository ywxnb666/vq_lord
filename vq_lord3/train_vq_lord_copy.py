...
def _resolve_stage3_sample_cache_path(args) -> str:
    cache_path = str(getattr(args, "stage3_sample_cache_path", "") or "").strip()
    if cache_path:
        return cache_path
    return os.path.join(str(getattr(args, "save_path", ".") or "."), "stage3_sample_cache.pt")
...
def _load_stage3_sample_cache(cache_path: str) -> Optional[List[Stage3SampleCacheItem]]:
    ...
    try:
        ...
        print(f"[Stage3][Cache][Warn] 读取 sample cache 失败，将重建: {cache_path} ({exc})")
        return None
    ...
    if isinstance(payload, list):
        ...
        print(f"[Stage3][Cache][Warn] sample cache 格式无效，将重建: {cache_path}")
        return None
    if cache_items:
        probe = cache_items[0]
        missing_fields = []
        ...
        if missing_fields:
            print(
                f"[Stage3][Cache][Warn] sample cache 缺少字段 {missing_fields}，将重建: {cache_path}"
            )
            return None
...
def _save_stage3_sample_cache(cache_path: str, cache_items: List[Stage3SampleCacheItem]):
    ...
    try:
        ...
        print(f"[Stage3][Cache][Warn] 保存 sample cache 失败，忽略: {cache_path} ({exc})")
...
def _evaluate_stage3_answer_metrics(
    ...
    eval_mode = str(getattr(args, "stage3_eval_answer_mode", "logits") or "logits").strip().lower()
    model.eval()
...
def _get_stage3_phase_a_batch_size(args, train_bucket_meta: Optional[dict] = None) -> int:
    ...
    if int(getattr(args, "stage3_bucket_batch_size", 0)) > 0:
        batch_size = int(getattr(args, "stage3_bucket_batch_size", 0))
...
def _build_stage3_static_batches(
    ...
    use_bucket = (
        ...
        and not bool(int(getattr(args, "disable_bucket_for_stage3", 1)))
    )
    ...
    if use_bucket:
        ...
        for idx in active_indices:
            bucket_key = sample_to_bucket.get(int(idx), "__stage3_fallback__")
    ...
    try:
        ...
        for batch_indices in tqdm(stage3_batches, desc="Stage3 Phase-A"):
            ...
            for row_idx, idx in enumerate(batch_indices):
                ...
                if bool(int(getattr(args, "stage3_pair_use_answer_correctness", 1))) and tokenizer is not None:
...
def _run_one_period_train(
    ...
    obj_weight = float(getattr(args, "stage3_obj_weight", 1.0))
    reg_weight = float(getattr(args, "stage3_reg_weight", 1.0))
    answer_anchor_weight = float(getattr(args, "stage3_answer_anchor_weight", 1.0))
    mc_weight = float(getattr(args, "stage3_mc_weight", 1.0))
    field_weights = {
...
def train_stage3_lord(model, train_dataset, args, tb_writer, train_bucket_meta: Optional[dict] = None):
    ...
    stage3_use_static_bucket = bool(
        getattr(args, "dataset_name", "") == "scienceqa"
        and train_bucket_meta is not None
        and not bool(int(getattr(args, "disable_bucket_for_stage3", 1)))
    )
    stage3_eval_answer_mode = str(getattr(args, "stage3_eval_answer_mode", "logits") or "logits").strip().lower()
    ...
    print(
        ...
        f"mc_w={float(getattr(args, 'stage3_mc_weight', 1.0)):.2f}, "
        f"obj_w={float(getattr(args, 'stage3_obj_weight', 1.0)):.2f}, "
        f"reg_w={float(getattr(args, 'stage3_reg_weight', 1.0)):.2f}, "
        f"ans_anchor_w={float(getattr(args, 'stage3_answer_anchor_weight', 1.0)):.2f}, "
        f"pair_by_answer={int(bool(getattr(args, 'stage3_pair_use_answer_correctness', 1)))}, "
        f"wrong_image={int(bool(getattr(args, 'stage3_wrong_image_enable', 0)))}, "
        f"force_cold_start_p0={int(bool(getattr(args, 'stage3_force_cold_start_period0', 1)))}, "
        ...
        for period_idx in range(resume_period_idx, period_num):
            ...
            training_items, next_states, cold_start_count, swap_count, plus_lp_sum, minus_lp_sum = _build_pairs_and_next_states(
                ...
                force_vic_positive=bool(
                    period_idx == 0 and int(getattr(args, "stage3_force_cold_start_period0", 1))
                ),
                train_bucket_meta=train_bucket_meta,
                tokenizer=tokenizer,
            )
            ...
            avg_mc_weighted = float(getattr(args, "stage3_mc_weight", 1.0)) * avg_mc
            avg_obj_weighted = float(getattr(args, "stage3_obj_weight", 1.0)) * avg_obj
            avg_reg_weighted = float(getattr(args, "stage3_reg_weight", 1.0)) * avg_reg
            avg_answer_anchor_weighted = (
                float(getattr(args, "stage3_answer_anchor_weight", 1.0)) * avg_answer_anchor
            )
            avg_wrong_weighted = float(getattr(args, "stage3_wrong_image_weight", 0.2)) * avg_wrong
            period_counter += 1
            print(
