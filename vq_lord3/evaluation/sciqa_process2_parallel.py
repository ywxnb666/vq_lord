...
功能:
1. 读取 vq_lord3/data/preprocess/sciqa_preprocess.py 生成的 bucket plan JSON
...
from vq_lord3.evaluation.sciqa_process2 import (
...
from vq_lord3.data.iconqa_mcq import materialize_iconqa_image
...
def run_stage2_aligned_logits_shard(
    model,
    processor,
    bucket_payload: dict,
    ...
    eval_dataset = ScienceQADataset(
        ...
        seed=seed,
        teacher_lang="zh",
        stage2_vic_include_context=False,
        require_teacher_annotation=False,
    )
    sample_cache = _build_stage2_sample_cache(eval_dataset)
    ...
    for batch in tqdm(shard_plan, desc="ScienceQA Stage2-Aligned Logits Eval"):
    ...
    metrics = {
        ...
        "eval_impl": "stage2_aligned_logits",
    }
    ...
    print(f"[Stage2-Aligned Shard] Accuracy: {accuracy:.4f} ({correct}/{total})")
    print(f"[Stage2-Aligned Shard] Results saved to: {save_path}")
...
def run_eval_shard(
    ...
    stage2_llava_remove_context: int = 0,
    run_config: Optional[dict] = None,
):
    dataset_name = normalize_dataset_name(dataset_name)
    student_model_type = normalize_student_model_type(student_model_type)
    llava_remove_context = student_model_type == LLAVA_NEXT and int(stage2_llava_remove_context) == 1
    ...
    if answer_mode == "logits":
        run_stage2_aligned_logits_shard(
            model=model,
            processor=processor,
...
def merge_shard_results(
    ...
    if (run_config or {}).get("answer_mode") == "generate_readable":
        ...
        use_no_context = (
            ...
            and int((run_config or {}).get("stage2_llava_remove_context", 0)) == 1
        )
        field_names = (
            ("observed_facts", "reasoning", "answer")
            if use_no_context
...
def main():
    ...
    if int(args.merge_only) == 1:
        merge_run_config = {
            ...
            "stage2_llava_remove_context": int(args.stage2_llava_remove_context),
            "prompt_style": (
                "llava_three_field_no_context"
                if (
                    args.student_model_type == LLAVA_NEXT
                    and args.answer_mode == "generate_readable"
                    and int(args.stage2_llava_remove_context) == 1
                )
