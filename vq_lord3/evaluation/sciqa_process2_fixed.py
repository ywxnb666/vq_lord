...
ScienceQA 多模态验证脚本（旧 Stage2 prompt 口径）

功能:
1. 加载 LLaVA/LLaVA-Next 学生模型（可选 LoRA 适配器）
2. 使用旧 Stage2/ScienceQADataset prompt 模板进行推理
3. 解析模型输出并计算选择题准确率
...
def add_vq_inference_hook(model, vq_codebook_size: int, freeze_vision_tower: int):
    ...
    patch_llava_next_get_image_features_for_vq(base_model)
    return model


def patch_llava_next_get_image_features_for_vq(base_model):
    """
    最小推理修复：
    当前 transformers 版本默认读取 hidden_states[vision_feature_layer]，
    而 VQVisionEncoder 仅替换 last_hidden_state。
    这里在 use_vq 推理路径下，显式改为优先读取 last_hidden_state，
    确保 projector 真正接收到 VQ 重构特征。
    """
    if getattr(base_model, "_vq_get_image_features_patched", False):
        return
    ...
    def _patched_get_image_features(
        ...
        if pixel_values.dim() == 5:
            pixel_values_list = [pix_val[:num_patch] for pix_val, num_patch in zip(pixel_values, image_num_patches)]
            pixel_values = torch.cat(pixel_values_list, dim=0)
        elif pixel_values.dim() != 4:
            raise ValueError(f"pixel_values of shape {pixel_values.shape}, expect to be of 4 or 5 dimensions")

        image_features = self.vision_tower(pixel_values, output_hidden_states=True)
        if hasattr(image_features, "last_hidden_state") and image_features.last_hidden_state is not None:
    ...
    base_model._vq_get_image_features_patched = True
...
def parse_args():
    parser = argparse.ArgumentParser(description="ScienceQA 多模态验证脚本（旧 Stage2 prompt 口径）")
