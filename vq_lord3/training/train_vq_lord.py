...
from vq_lord3.data.iconqa_mcq import build_iconqa_mcq_samples, materialize_iconqa_image
from vq_lord3.models.student_models import (
    LLAVA_NEXT,
...
# Keep legacy flat-module imports working for older entrypoints.
sys.modules.setdefault("train_vq_lord", sys.modules[__name__])
...
def setup_args():
    ...
    parser.add_argument("--stage", type=int, default=2,
                       choices=[0, 1, 2],
                       help="训练阶段 (0=VQ预训练, 1=视觉蒸馏, 2=联合训练)")
    ...
    parser.add_argument("--stage1_rationale_weight", type=float, default=0.3,
                       help="Stage1 解释监督权重")
    parser.add_argument("--stage1_field_weight_observed", type=float, default=1.0,
                       help="Stage1 四字段 Observed Facts token 权重")
    parser.add_argument("--stage1_field_weight_context", type=float, default=1.0,
                       help="Stage1 四字段 Context token 权重")
    parser.add_argument("--stage1_field_weight_reasoning", type=float, default=0.5,
                       help="Stage1 四字段 Reasoning token 权重")
    parser.add_argument("--stage1_field_weight_answer", type=float, default=6.0,
                       help="Stage1 四字段 Answer token 权重")
    parser.add_argument("--stage1_answer_prefix_weight", type=float, default=12.0,
                       help="Stage1 Answer: 字段名前缀 token 权重")
    ...
    parser.add_argument("--stage2_projector_lr_scale", type=float, default=0.1,
                       help="Stage2 projector 学习率相对 Stage2 LoRA 学习率的缩放")
    ...
    parser.add_argument("--stage2_eval_max_new_tokens", type=int, default=512,
                       help="Stage2 内置 generate_readable 评估最大生成 token 数")
    ...
    parser.add_argument("--stage2_eval_answer_mode", type=str, default="generate_readable",
                       choices=["generate", "logits", "generate_readable"],
                       help="Stage2 内置评估口径：generate_readable 为四字段生成主评测")
    ...
    parser.add_argument("--stage2_field_weight_reasoning", type=float, default=1.2,
                       help="Stage2 教师正则中 reasoning 的 token 权重")
    parser.add_argument("--stage2_field_weight_answer", type=float, default=2.0,
                       help="Stage2 教师正则中 answer 的 token 权重")
    parser.add_argument("--stage2_obj_weight", type=float, default=0.1,
                       help="Stage2 偏好目标 L_obj 的总权重")
    parser.add_argument("--stage2_reg_weight", type=float, default=0.3,
                       help="Stage2 教师正则 L_reg 的总权重")
    parser.add_argument("--stage2_readable_sft_weight", type=float, default=1.0,
                       help="Stage2 y_vic 四字段显式 SFT anchor 权重")
    ...
    parser.add_argument("--stage2_answer_prefix_weight", type=float, default=1.0,
                       help="Stage2 Answer 字段开头 token 直接 SFT 监督权重")
    parser.add_argument("--stage2_mc_weight", type=float, default=0.0,
                       help="Stage2 选择题主损失权重（基于 Answer: 后下一 token logits 的多选 CE）")
    parser.add_argument("--stage2_pair_use_answer_correctness", type=int, default=0,
    ...
    parser.add_argument("--stage2_llava_remove_context", type=int, default=0,
                       help="Stage2 LLaVA 是否使用三字段 no-context readable 目标，1=启用，0=关闭")
    ...
    parser.add_argument("--stage1_resume_path", type=str, default="",
                       help="Stage1 断点目录，非空时从该目录恢复训练")
    parser.add_argument("--stage1_resume_save_path", type=str, default="",
                       help="Stage1 断点保存目录，默认 save_path/stage1_resume_latest")
    parser.add_argument("--stage1_resume_save_optimizer", type=int, default=1,
                       help="Stage1 断点是否保存 optimizer state（1=保存,0=不保存）")
    parser.add_argument("--stage1_resume_save_interval", type=int, default=1,
                       help="Stage1 每隔多少个 epoch 保存一次断点")
    ...
    parser.add_argument("--save_each_epoch", type=int, default=0,
                       help="是否在 Stage0/1/2 每个 epoch 结束时额外保存一次检查点")
...
def save_stage1_checkpoint(model, args, ckpt_path: str):
    ...
    config_info = {
        ...
        "beta": args.beta,
        "stage": 1,
        "stage1_objective": "readable_four_field_generate",
        ...
        "stage1_answer_format": "teacher_annotation.answer",
...
def load_stage1_checkpoint(model, ckpt_path: str, remove_vq_codebook: bool = False):
    """加载 Stage1 检查点"""
    model = unwrap_model(model)
    ...
    if os.path.exists(config_path):
        ...
        print(
            ...
            f"objective={stage1_cfg.get('stage1_objective')}, "
            f"field_w=({stage1_cfg.get('stage1_field_weight_observed')}, "
            f"{stage1_cfg.get('stage1_field_weight_context')}, "
            f"{stage1_cfg.get('stage1_field_weight_reasoning')}, "
            f"{stage1_cfg.get('stage1_field_weight_answer')}), "
            f"answer_prefix_w={stage1_cfg.get('stage1_answer_prefix_weight')}"
        )
    else:
...
def _stage1_contract_missing_files(ckpt_path: str, require_vq: bool = True) -> List[str]:
    required_files = [
        "projector.pt",
        "stage1_config.json",
        "adapter_config.json",
...
def wrap_model_for_ddp(model, args, find_unused_parameters: bool = False):
    ...
    return DDP(
        ...
        # Stage1/Stage2 在 codebook 冻结时会显式禁止其更新。
        broadcast_buffers=False,
        find_unused_parameters=bool(find_unused_parameters),
    )


def wrap_model_for_stage1_ddp(model, args):
...
from vq_lord3.training.vq_lord_stage0 import train_stage0_vq
from vq_lord3.training.vq_lord_stage1 import (
    _get_stage1_vq_loss,
    _get_stage1_vq_stats,
    train_stage1_vision,
)
from vq_lord3.training.vq_lord_stage2 import (
    PeriodState,
    PeriodTrainingDataset,
    PeriodTrainingItem,
    Stage2SampleCacheItem,
    _bootstrap_period_states,
    _build_pairs_and_next_states,
    _build_stage2_choice_token_map,
    _build_stage2_sample_cache,
    _build_stage2_static_batches,
    ...
    _evaluate_stage2_answer_metrics,
...
def main():
    ...
    try:
        if (
            ...
            raise RuntimeError("当前未实现 Stage0 多卡训练；请先单卡准备 Stage0 codebook，再用多卡执行 Stage1/Stage2。")
        ...
        if is_main_process():
            ...
            if args.remove_vq_codebook:
                print("[Info] remove_vq_codebook=1，使用学生模型原生视觉链路并跳过 Stage0。")
        ...
        if _is_hf_multichoice_dataset(args.dataset_name):
            ...
            if int(args.stage) >= 1:
                if args.distributed:
                    if is_main_process():
                        print(f"加载 {args.dataset_name} 教师回答（stage>=1, rank0 负责补齐缓存）...")
                    ...
                    if is_main_process():
                        print(f"加载 {args.dataset_name} 教师回答（stage>=1）...")
                    train_samples = attach_gpt4v_teacher_responses(train_samples, args)
            elif is_main_process():
                print("stage<1，跳过教师回答采集/补齐")
        ...
        def build_train_dataloader(stage_id: int):
            ...
            if use_bucket:
                ...
                if args.distributed and stage_id == 1:
        ...
        if args.stage >= 0 and not bool(args.remove_vq_codebook):
            stage0_dataloader = build_train_dataloader(stage_id=0)
            if args.distributed and not os.path.exists(args.vq_codebook_path):
                raise RuntimeError(
                    "当前为 Stage1 多卡模式，但未找到可复用的 Stage0 codebook。"
                    f"请先单卡完成 Stage0，期望路径: {args.vq_codebook_path}"
                )
        ...
        if args.stage >= 1:
            ...
            has_stage1_resume = bool(str(args.stage1_resume_path).strip())
            if has_stage1_resume:
                if is_main_process():
                    print(f"检测到Stage1 resume路径，优先从断点恢复训练: {args.stage1_resume_path}")
                model = wrap_model_for_stage1_ddp(model, args)
                ...
                args.reuse_stage1
                and os.path.exists(args.stage1_ckpt_path)
                and (
                    bool(args.remove_vq_codebook)
                    or os.path.exists(os.path.join(args.stage1_ckpt_path, "vq_codebook.pt"))
                )
            ):
                if is_main_process():
                    print(f"检测到Stage1检查点，跳过Stage1并加载: {args.stage1_ckpt_path}")
                model = load_stage1_checkpoint(
                    model,
        ...
        if args.stage >= 2:
            ...
            missing_stage1_files = _stage1_contract_missing_files(
                args.stage1_ckpt_path,
            ...
            if is_main_process():
                save_stage2_checkpoint(
                    model,
                    os.path.join(args.save_path, "stage2_lord_final"),
                    remove_vq_codebook=bool(args.remove_vq_codebook),
                    student_model_type=str(getattr(args, "student_model_type", "llava_next")),
                )
            barrier_if_distributed()
