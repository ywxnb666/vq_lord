...
def _stage2_masked_mean_ddp(
    ...
    return _stage2_global_mean_from_local_sum(local_sum, local_weight)


def _stage2_prefix_mask_from_field_mask(field_mask: torch.Tensor, max_prefix_tokens: int = 4) -> torch.Tensor:
    active = (field_mask > 0).to(dtype=field_mask.dtype)
    prefix_pos = torch.cumsum(active, dim=1)
    return active * (prefix_pos <= int(max_prefix_tokens)).to(dtype=field_mask.dtype)


def _stage2_observed_content_mask(observed_mask: torch.Tensor, max_prefix_tokens: int = 4) -> torch.Tensor:
    prefix_mask = _stage2_prefix_mask_from_field_mask(
        observed_mask,
...
def _stage2_row_masked_mean(values: torch.Tensor, mask: torch.Tensor) -> tuple[torch.Tensor, torch.Tensor]:
...
def _stage2_valid_row_mean_ddp(values: torch.Tensor, valid: torch.Tensor) -> tuple[torch.Tensor, float]:
...
def _stage2_use_answer_correctness_pair(args) -> bool:
    return str(getattr(args, "student_model_type", "")) == "llava_next"


def _stage2_llava_remove_context(args) -> bool:
    return (
        str(getattr(args, "student_model_type", "")) == "llava_next"
        and int(getattr(args, "stage2_llava_remove_context", 0)) == 1
    )


def _stage2_llava_readable_sft_mask(
...
def _stage2_teacher_answer_letter(sample: "Stage2SampleCacheItem", tokenizer) -> str:
...
def _stage2_score_readable_candidate(
    cand: dict,
...
def _stage2_resume_config(args) -> dict:
    return {
        ...
        "stage2_vic_include_context": int(getattr(args, "stage2_vic_include_context", 0)),
        "lr": float(args.lr),
        "stage2_lr_scale": float(getattr(args, "stage2_lr_scale", 0.0)),
        "stage2_projector_lr_scale": float(getattr(args, "stage2_projector_lr_scale", 0.0)),
        "stage2_grad_clip": float(getattr(args, "stage2_grad_clip", 0.0)),
        ...
        "stage2_bucket_batch_size": int(getattr(args, "stage2_bucket_batch_size", 0)),
        "disable_bucket_for_stage2": int(getattr(args, "disable_bucket_for_stage2", 0)),
        ...
        "stage2_eval_max_new_tokens": int(getattr(args, "stage2_eval_max_new_tokens", 0)),
        "stage2_train_projector": int(getattr(args, "stage2_train_projector", 0)),
        "stage2_field_weight_observed": float(getattr(args, "stage2_field_weight_observed", 0.0)),
        "stage2_field_weight_context": float(getattr(args, "stage2_field_weight_context", 0.0)),
        "stage2_field_weight_reasoning": float(getattr(args, "stage2_field_weight_reasoning", 0.0)),
        "stage2_field_weight_answer": float(getattr(args, "stage2_field_weight_answer", 0.0)),
        "stage2_obj_weight": float(getattr(args, "stage2_obj_weight", 0.0)),
        "stage2_reg_weight": float(getattr(args, "stage2_reg_weight", 0.0)),
        "stage2_readable_sft_weight": float(getattr(args, "stage2_readable_sft_weight", 1.0)),
        "stage2_answer_anchor_weight": float(getattr(args, "stage2_answer_anchor_weight", 0.0)),
        "stage2_answer_prefix_weight": float(getattr(args, "stage2_answer_prefix_weight", 0.0)),
        "stage2_mc_weight": float(getattr(args, "stage2_mc_weight", 0.0)),
        "stage2_pair_use_answer_correctness": int(getattr(args, "stage2_pair_use_answer_correctness", 0)),
        "stage2_wrong_image_enable": int(getattr(args, "stage2_wrong_image_enable", 0)),
        "stage2_wrong_image_weight": float(getattr(args, "stage2_wrong_image_weight", 0.0)),
        "stage2_wrong_image_margin": float(getattr(args, "stage2_wrong_image_margin", 0.0)),
        "stage2_force_cold_start_period0": int(getattr(args, "stage2_force_cold_start_period0", 0)),
        "stage2_llava_remove_context": int(getattr(args, "stage2_llava_remove_context", 0)),
        "scienceqa_preprocessed_path": str(getattr(args, "scienceqa_preprocessed_path", "")),
        "stage2_sample_cache_path": str(getattr(args, "stage2_sample_cache_path", "")),
        "vq_codebook_path": str(getattr(args, "vq_codebook_path", "")),
        "remove_vq_codebook": int(getattr(args, "remove_vq_codebook", 0)),
        "stage1_ckpt_path": str(getattr(args, "stage1_ckpt_path", "")),
...
def _validate_stage2_resume_config(resume_config: dict, args):
    ...
    strict_keys = [
        ...
        "stage2_vic_include_context",
        "disable_bucket_for_stage2",
        "sub_set_num",
        "tau1",
        "tau_delta",
        "temperature",
        "max_new_tokens",
        "stage2_eval_max_new_tokens",
        "stage2_train_projector",
        "stage2_projector_lr_scale",
        ...
        "stage2_obj_weight",
        "stage2_reg_weight",
        "stage2_readable_sft_weight",
        "stage2_answer_anchor_weight",
        "stage2_answer_prefix_weight",
        "stage2_mc_weight",
        "stage2_pair_use_answer_correctness",
        "stage2_wrong_image_enable",
        "stage2_wrong_image_weight",
        "stage2_wrong_image_margin",
        "stage2_force_cold_start_period0",
        "stage2_llava_remove_context",
        "remove_vq_codebook",
        "stage1_ckpt_path",
    ]
    ...
    for key in strict_keys:
        if key not in resume_config:
            raise RuntimeError(
                f"Stage2 resume 配置缺少必需字段: key={key}。"
                "请使用新规范重新启动 Stage2，不兼容旧 resume。"
            )
        if resume_config[key] != current[key]:
            raise RuntimeError(
...
def _stage2_phase_name_offset(phase_name: str) -> int:
...
def _make_stage2_phase_seed(
    ...
    phase_offset = _stage2_phase_name_offset(phase_name)
    seed = (
...
def _make_stage2_torch_generator(device: str, seed: int) -> torch.Generator:
...
def _shard_stage2_global_batches(global_batches: List[List[int]]) -> List[Tuple[int, List[int]]]:
    return [
...
def _stage2_gather_object_to_main(local_obj):
...
def _stage2_broadcast_object_from_main(obj):
    if not is_distributed():
        return obj
...
def _is_stage2_qwen_backend(args) -> bool:
...
def _build_stage2_period_dataloader(
...
def _validate_stage2_chunk_merge(
    ...
    if merged_sorted != active_sorted:
        raise RuntimeError(
            f"[Stage2][DDP] {desc} merge 后 sample_idx 覆盖不完整: "
...
def _stack_pixel_values_for_backend(pv_list: List[torch.Tensor], args) -> torch.Tensor:
    if _is_stage2_qwen_backend(args):
...
def _build_vic_field_masks(
    ...
    if missing:
        if len(field_lines) != 4:
            raise RuntimeError(f"Stage2 y_vic 字段 mask 构建失败: missing={missing}, target={vic_target!r}")
        observed_mask.zero_()
        context_mask.zero_()
        ...
        if still_missing:
            raise RuntimeError(f"Stage2 y_vic 字段 mask 构建失败: missing={still_missing}, target={vic_target!r}")
    return observed_mask, context_mask, reasoning_mask, answer_mask, other_mask
...
def _build_vic_field_masks_no_context(
    ...
    if missing:
        if len(field_lines) != 3:
            raise RuntimeError(f"Stage2 y_vic no-context 字段 mask 构建失败: missing={missing}, target={vic_target!r}")
        observed_mask.zero_()
        reasoning_mask.zero_()
        answer_mask.zero_()
        max_target_len = max(0, old_len - max(0, int(prompt_len) - 1))
        ...
        if still_missing:
            raise RuntimeError(
                f"Stage2 y_vic no-context 字段 mask 构建失败: missing={still_missing}, target={vic_target!r}"
            )
    return observed_mask, context_mask, reasoning_mask, answer_mask, other_mask


def _build_stage2_sample_cache(train_dataset, args=None) -> List[Stage2SampleCacheItem]:
    cache: List[Stage2SampleCacheItem] = []
    processor = train_dataset.processor
    tokenizer = train_dataset.processor.tokenizer
    llava_remove_context = _stage2_llava_remove_context(args) if args is not None else False
        ...
        if llava_remove_context:
            ...
            if isinstance(ann, dict):
                ...
                if bool(getattr(train_dataset, "require_teacher_annotation", False)):
                    raise RuntimeError("Stage2 LLaVA no-context 训练要求 teacher_annotation 存在。")
                answer_letter = str(raw_item.get("answer_letter", "") or "").strip().upper()[:1]
                if not ("A" <= answer_letter <= "Z"):
                    raise RuntimeError("Stage2 LLaVA no-context eval cache 缺少可用 answer_letter。")
                observed = "unavailable"
                reasoning = "unavailable"
                answer = answer_letter
            if not observed or not reasoning or not answer:
                raise RuntimeError("Stage2 LLaVA no-context 教师字段存在空值，无法构造训练目标。")
            vic_target = "\n".join([
...
def _resolve_stage2_sample_cache_path(args) -> str:
    cache_path = str(args.stage2_sample_cache_path).strip()
    if not cache_path:
        raise RuntimeError("stage2_sample_cache_path 不能为空")
    return cache_path
...
def _sanitize_stage2_cache_name(value: str) -> str:
    return re.sub(r"[^A-Za-z0-9_.-]+", "_", str(value or "").strip()).strip("_")


def _stage2_expected_cache_model_name(args) -> str:
    model_path = str(getattr(args, "model_path", "") or "").rstrip("/")
    model_name = os.path.basename(model_path)
    model_name = _sanitize_stage2_cache_name(model_name)
    if model_name:
        return model_name
    return _sanitize_stage2_cache_name(getattr(args, "student_model_type", ""))


def _stage2_sample_cache_name_matches_model(args, cache_path: str) -> bool:
    expected = _stage2_expected_cache_model_name(args)
    if not expected:
        return True
    cache_name = os.path.basename(str(cache_path or ""))
    if expected in cache_name:
        return True
    print(
        f"[Stage2][Cache] sample cache 文件名不匹配当前学生模型，将重建: "
        f"path={cache_path}, expected_model_name={expected}"
    )
    return False


def _load_stage2_sample_cache(cache_path: str, args=None) -> Optional[List[Stage2SampleCacheItem]]:
    if not cache_path or not os.path.exists(cache_path):
        return None
    if args is not None and not _stage2_sample_cache_name_matches_model(args, cache_path):
        return None
    ...
    if isinstance(payload, list):
        ...
        raise RuntimeError(f"[Stage2][Cache] sample cache 格式无效: {cache_path}")
    if cache_items:
        probe = cache_items[0]
        for field_name in (
            "prompt_ids",
            ...
            if not hasattr(probe, field_name):
                raise RuntimeError(
                    f"[Stage2][Cache] sample cache 缺少字段 {field_name}: {cache_path}"
                )
...
def _build_stage2_optimizer_param_groups(model, args, stage2_lr: float) -> List[dict]:
    base_model = unwrap_model(model)
    ...
    if lora_params:
        param_groups.append({"params": lora_params, "lr": stage2_lr})
    if projector_params:
        projector_lr = stage2_lr * float(getattr(args, "stage2_projector_lr_scale", 0.1))
...
def _generate_candidate_single(
    ...
    gen_kwargs = dict(
        ...
        max_new_tokens=int(args.max_new_tokens if max_new_tokens is None else max_new_tokens),
    ...
    generated_mask = torch.ones_like(generated, dtype=torch.long)
...
def _generate_candidate_batch(
    ...
    for row_idx in range(generated.shape[0]):
        ...
        left_pad_len = int(prompt_ids.shape[1]) - int(samples[row_idx].prompt_len)
        if left_pad_len > 0:
            row = row[left_pad_len:]
        non_pad = (row != int(pad_token_id)).nonzero(as_tuple=False).view(-1)
        keep_len = int(non_pad[-1].item()) + 1 if non_pad.numel() > 0 else 1
        row = row[:keep_len]
        row_mask = torch.ones_like(row, dtype=torch.long)
...
def _parse_readable_answer_field(text: str, remove_context: bool = False) -> Tuple[bool, str]:
    ...
    for idx, prefix in enumerate(prefixes):
        ...
        end = positions[idx + 1] if idx + 1 < len(prefixes) else len(str(text or ""))
        fields.append(str(text or "")[start:end].strip())
    if not all(fields):
        return False, ""
...
def _evaluate_stage2_answer_metrics(
    ...
    model = unwrap_model(model)
    eval_mode = str(args.stage2_eval_answer_mode).strip().lower()
    local_eval_indices = list(eval_indices)
    if is_distributed():
        local_eval_indices = [
            idx for pos, idx in enumerate(eval_indices)
            if (pos % get_world_size()) == get_rank()
        ]
    model.eval()
    ...
    try:
        ...
        if eval_mode == "logits" and choice_token_map is not None:
            batch_size = max(1, int(args.batch_size))
            sorted_indices = sorted(local_eval_indices, key=lambda idx: _stage2_static_sort_key(sample_cache[idx]))
            ...
            for idx in local_eval_indices:
                ...
                ids, _ = _generate_candidate_single(
                    ...
                    do_sample=False,
                    max_new_tokens=int(args.stage2_eval_max_new_tokens),
                )
                prompt_len = int(sample.prompt_len)
                gen_ids = ids[prompt_len:] if ids.shape[0] > prompt_len else ids
                ...
                if eval_mode == "generate_readable":
                    readable_ok, answer_text = _parse_readable_answer_field(
                        text,
                        remove_context=_stage2_llava_remove_context(args),
                    )
    ...
    stats = torch.tensor(
        [float(acc_hits), float(fmt_hits), float(total)],
        device=args.device,
        dtype=torch.float64,
    )
    if is_distributed():
...
def _get_stage2_phase_a_batch_size(args, train_bucket_meta: Optional[dict] = None) -> int:
    ...
    if int(args.stage2_bucket_batch_size) > 0:
        batch_size = int(args.stage2_bucket_batch_size)
...
def _build_stage2_static_batches(
    ...
    if use_bucket:
        if train_bucket_meta is None:
            raise RuntimeError("Stage2 需要有效的 ScienceQA 分桶预处理结果")
        ...
        for idx in active_indices:
            if int(idx) not in sample_to_bucket:
                raise RuntimeError(f"Stage2 缺少 sample_id={int(idx)} 的 bucket 映射")
...
def _bootstrap_period_states_distributed(
    ...
    base_model = unwrap_model(model)
    global_batches = _build_stage2_static_batches(
        active_indices=active_indices,
        sample_cache=sample_cache,
        args=args,
        train_bucket_meta=train_bucket_meta,
    )
    local_batch_plan = _shard_stage2_global_batches(global_batches)
    local_chunks = []
    ...
    try:
        for global_batch_idx, batch_indices in tqdm(
            local_batch_plan,
            desc=f"Stage2 bootstrap rank{get_rank()}",
            ...
            cand1_outputs = _generate_candidate_batch(
                ...
                image_token_id,
                pad_token_id,
                generator=_make_stage2_torch_generator(
                    args.device,
                    _make_stage2_phase_seed(
    ...
    gathered = _stage2_gather_object_to_main(local_chunks)
    merged_states = None
    if is_main_process():
        merged_states = {}
        ordered = []
        ...
        _validate_stage2_chunk_merge(merged_sample_indices, active_indices, "bootstrap")
    merged_states = _stage2_broadcast_object_from_main(merged_states)
...
def _build_pairs_and_next_states_distributed(
    ...
    if is_main_process():
        print(
            "[Stage2] wrong-image pairing stats: "
    ...
    try:
        for global_batch_idx, batch_indices in tqdm(
            local_batch_plan,
            desc=f"Stage2 Phase-A rank{get_rank()}",
            disable=not is_main_process(),
        ):
            batch_samples = [sample_cache[idx] for idx in batch_indices]
            ...
            for row_idx, idx in enumerate(batch_indices):
                ...
                if _stage2_use_answer_correctness_pair(args) and tokenizer is not None:
                    teacher_letter = _stage2_teacher_answer_letter(sample, tokenizer)
                    if len(teacher_letter) == 1 and "A" <= teacher_letter <= "Z":
                        cand_scored = [
                            _stage2_score_readable_candidate(
                                cand,
                                sample,
                                tokenizer,
                                teacher_letter,
    ...
    gathered_training = _stage2_gather_object_to_main(local_training_chunks)
    gathered_states = _stage2_gather_object_to_main(local_state_chunks)
    gathered_stats = _stage2_gather_object_to_main(local_stats)
    ...
    if is_main_process():
        ...
        _validate_stage2_chunk_merge(merged_sample_indices, active_indices, "training_items")
        _validate_stage2_chunk_merge(merged_state_indices, active_indices, "next_states")
        if len(merged_training_items) != len(active_indices):
            raise RuntimeError(
                f"[Stage2][DDP] training_items 长度异常: "
    ...
    merged_training_items = _stage2_broadcast_object_from_main(merged_training_items)
    merged_next_states = _stage2_broadcast_object_from_main(merged_next_states)
    merged_stats = _stage2_broadcast_object_from_main(merged_stats)
    return (
        merged_training_items,
        merged_next_states,
...
def _run_one_period_train(
    ...
    model.train()
    grad_accum = max(1, int(args.stage2_grad_accum or args.grad_accum))
    grad_clip = float(args.stage2_grad_clip)
    wrong_image_enable = bool(int(args.stage2_wrong_image_enable))
    wrong_image_weight = float(args.stage2_wrong_image_weight)
    wrong_image_margin = float(args.stage2_wrong_image_margin)
    obj_weight = float(args.stage2_obj_weight)
    reg_weight = float(args.stage2_reg_weight)
    readable_sft_weight = float(getattr(args, "stage2_readable_sft_weight", 1.0))
    answer_anchor_weight = float(args.stage2_answer_anchor_weight)
    answer_prefix_weight = float(args.stage2_answer_prefix_weight)
    mc_weight = float(args.stage2_mc_weight) if _stage2_use_answer_correctness_pair(args) else 0.0
    field_weights = {
        "observed": float(args.stage2_field_weight_observed),
        "context": float(args.stage2_field_weight_context),
        "reasoning": float(args.stage2_field_weight_reasoning),
        "answer": float(args.stage2_field_weight_answer),
    }
    ...
    progress_bar = tqdm(
        loader,
        desc=f"Stage2 S{sub_stage_idx+1} P{period_idx+1}",
        disable=not is_main_process(),
    )
    for batch_idx, batch in enumerate(progress_bar):
        ...
        loss_plus, loss_plus_val = _stage2_masked_mean_ddp(
            log_clip(old_lp_plus - cur_lp_plus),
            mask_plus,
        )
        if _sync_skip(
            not torch.isfinite(loss_plus).item(),
            f"[Stage2][Warn] step={global_step} 检测到非有限 plus loss，所有 rank 同步跳过该 batch。",
        ):
            continue
        ...
        loss_minus, loss_minus_val = _stage2_masked_mean_ddp(
            log_clip(-old_lp_minus + cur_lp_minus),
            mask_minus,
        )
        if _sync_skip(
            not torch.isfinite(loss_minus).item(),
            f"[Stage2][Warn] step={global_step} 检测到非有限 minus loss，所有 rank 同步跳过该 batch。",
        ):
            continue
        with _backward_sync_context():
        ...
        loss_reg, loss_reg_val = _stage2_masked_mean_ddp(delta_vic, weighted_mask_vic)
        readable_sft_mask = weighted_mask_vic
        if _stage2_use_answer_correctness_pair(args):
            readable_sft_mask = _stage2_llava_readable_sft_mask(
                weighted_mask_vic,
                field_masks["reasoning"],
            )
        loss_readable_sft, loss_readable_sft_val = _stage2_masked_mean_ddp(
            -cur_lp_vic,
            readable_sft_mask,
        )
        field_loss_vals = {}
        for name, field_mask in field_masks.items():
            _, field_loss_val = _stage2_masked_mean_ddp(delta_vic, field_mask)
            field_loss_vals[name] = field_loss_val
            total_field[name] += field_loss_vals[name]
        answer_anchor_mask = field_masks["answer"]
        loss_answer_anchor, loss_answer_anchor_val = _stage2_masked_mean_ddp(
            -cur_lp_vic,
            answer_anchor_mask,
        )
        answer_prefix_mask = _stage2_prefix_mask_from_field_mask(answer_anchor_mask)
        loss_answer_prefix, loss_answer_prefix_val = _stage2_masked_mean_ddp(
            -cur_lp_vic,
            answer_prefix_mask,
        )
        ...
        if wrong_image_enable:
            try:
                ...
                if is_main_process():
                    print(
                        f"[Stage2][Warn] step={global_step} wrong-image token/feature mismatch，"
            ...
            if cur_lp_wrong_vic is not None and cur_mask_wrong_vic is not None:
                ...
                observed_content_mask = _stage2_observed_content_mask(observed_mask_for_wrong)
                wrong_mask = wrong_mask * observed_content_mask * (1.0 - wrong_is_self).view(-1, 1)
                pos_score, pos_valid = _stage2_row_masked_mean(cur_lp_vic, wrong_mask)
                wrong_score, wrong_valid = _stage2_row_masked_mean(cur_lp_wrong_vic, wrong_mask)
                wrong_delta = F.relu(wrong_score - pos_score + wrong_image_margin)
                loss_wrong, loss_wrong_val = _stage2_valid_row_mean_ddp(
                    wrong_delta,
        ...
        loss_vic_total = (
            ...
            + reg_weight * loss_reg
            + answer_anchor_weight * loss_answer_anchor
            + answer_prefix_weight * loss_answer_prefix
            + wrong_image_weight * loss_wrong
        )
        if _sync_skip(
            not torch.isfinite(loss_vic_total).item(),
            f"[Stage2][Warn] step={global_step} 检测到非有限 vic loss，所有 rank 同步跳过该 batch。",
        ):
            continue
        with _backward_sync_context():
        ...
        if mc_weight > 0.0 and choice_token_map:
            ...
            if reduced_mc["has_mc"] > 0.0:
                ...
                if _sync_skip(
                    ...
                    f"[Stage2][Warn] step={global_step} 检测到非有限多选 loss，所有 rank 同步跳过该 batch。",
                ):
        ...
        loss_val = (
            mc_weight * mc_val
            + obj_weight * obj_val
            + readable_sft_weight * loss_readable_sft_val
            + reg_weight * reg_val
            + answer_anchor_weight * answer_anchor_val
            + answer_prefix_weight * loss_answer_prefix_val
        ...
        if (batch_idx + 1) % grad_accum == 0 or batch_idx == len(loader) - 1:
            ...
            if reduced_grad["invalid_grad"] > 0.0:
                if is_main_process():
                    print(
                        f"[Stage2][Warn] step={global_step} 检测到非有限梯度范数，"
...
def train_stage2_lord(model, train_dataset, args, tb_writer, train_bucket_meta: Optional[dict] = None):
    ...
    stage2_use_static_bucket = bool(_is_hf_multichoice_dataset(args.dataset_name) and train_bucket_meta is not None)
    stage2_eval_answer_mode = str(args.stage2_eval_answer_mode).strip().lower()
    readable_schema = (
        "llava_three_field_no_context"
        if _stage2_llava_remove_context(args)
    ...
    if is_main_process():
        print(
            ...
            f"projector_lr_scale={float(getattr(args, 'stage2_projector_lr_scale', 0.1)):.3f}, "
            f"resume_opt={int(bool(args.stage2_resume_save_optimizer))}, "
            f"eval_max_samples={int(args.stage2_eval_max_samples)}, "
            f"eval_max_new_tokens={int(args.stage2_eval_max_new_tokens)}, "
            f"field_w=({float(args.stage2_field_weight_observed):.2f},"
            f"{float(args.stage2_field_weight_context):.2f},"
            f"{float(args.stage2_field_weight_reasoning):.2f},"
            f"{float(args.stage2_field_weight_answer):.2f}), "
            f"mc_w={float(args.stage2_mc_weight):.2f}, "
            f"obj_w={float(args.stage2_obj_weight):.2f}, "
            f"readable_sft_w={float(getattr(args, 'stage2_readable_sft_weight', 1.0)):.2f}, "
            f"reg_w={float(args.stage2_reg_weight):.2f}, "
            f"ans_anchor_w={float(args.stage2_answer_anchor_weight):.2f}, "
            f"ans_prefix_w={float(args.stage2_answer_prefix_weight):.2f}, "
            f"pair_by_answer={int(_stage2_use_answer_correctness_pair(args))}, "
            f"schema={readable_schema}, "
            f"wrong_image={int(bool(args.stage2_wrong_image_enable))}, "
            f"force_cold_start_p0={int(bool(args.stage2_force_cold_start_period0))}, "
            f"phaseA_batch_size={stage2_phase_a_batch_size}, static_bucket={int(stage2_use_static_bucket)}, "
            f"eval_mode={stage2_eval_answer_mode}, distributed={int(is_distributed())}, "
    ...
    optimizer = torch.optim.AdamW(
        _build_stage2_optimizer_param_groups(model, args, stage2_lr),
        lr=stage2_lr,
    )
    ...
    if is_distributed():
        if is_main_process():
            sample_cache = _load_stage2_sample_cache(stage2_sample_cache_path, args)
            if sample_cache is None:
                sample_cache = _build_stage2_sample_cache(train_dataset, args=args)
    ...
    tokenizer = train_dataset.processor.tokenizer
    choice_token_map: Optional[Dict[int, List[int]]] = None
    if _stage2_use_answer_correctness_pair(args) and float(args.stage2_mc_weight) > 0.0:
        choice_token_map = _build_stage2_choice_token_map(tokenizer)
    answer_lookup: Dict[int, str] = {}
    ...
    stage2_eval_max_samples = int(args.stage2_eval_max_samples)
    ...
    stage2_eval_split = str(args.stage2_eval_scienceqa_split).strip()
    stage2_eval_path = str(args.stage2_eval_scienceqa_path or args.scienceqa_path).strip()
    stage2_eval_train_num = int(args.stage2_eval_train_num)
    ...
    if (
        ...
        if is_distributed():
            if is_main_process():
                eval_sample_cache = _load_stage2_sample_cache(stage2_eval_sample_cache_path, args)
                if eval_sample_cache is None:
                    eval_sample_cache = _build_stage2_sample_cache(eval_dataset, args=args)
        ...
        if not eval_sample_cache:
            raise RuntimeError("Stage2 eval sample cache 为空，无法开始 Stage2 评估")
    ...
    period_counter = 0
    resume_save_optimizer = bool(int(args.stage2_resume_save_optimizer))
    resume_save_interval = max(1, int(args.stage2_resume_save_interval))
    stage2_eval_every_period = max(1, int(args.stage2_eval_every_period))
    resume_save_dir = str(args.stage2_resume_save_path).strip()
    if not resume_save_dir:
        resume_save_dir = os.path.join(args.save_path, "stage2_resume_latest")

    resume_path = str(args.stage2_resume_path).strip()
    resume_progress = None
    if resume_path:
        resume_progress = _load_stage2_resume_state(model, optimizer, resume_path, args.device, args)
    ...
    if resume_progress:
        ...
        if bool(resume_progress.get("completed", False)):
            ...
            if is_main_process():
                print(
                    "[Stage2][Resume] 断点来自已完成训练，但当前 sub_stage_num 更大；"
    ...
    for sub_stage_idx in range(resume_sub_stage_idx, sub_stage_num):
        ...
        if is_resume_sub_stage and resume_period_idx > 0 and resume_period_states is not None:
            ...
            if is_main_process():
                print(
                    f"[Stage2][Resume] 继续 sub_stage={sub_stage_idx+1}, "
        ...
        for period_idx in range(resume_period_idx, period_num):
            ...
            if is_distributed():
                training_items, next_states, cold_start_count, swap_count, plus_lp_sum, minus_lp_sum = _build_pairs_and_next_states_distributed(
                    ...
                    force_vic_positive=bool(
                        period_idx == 0 and int(args.stage2_force_cold_start_period0)
                    ),
                    train_bucket_meta=train_bucket_meta,
                    tokenizer=tokenizer,
                    sub_stage_idx=sub_stage_idx,
            ...
            period_loader = _build_stage2_period_dataloader(
            ...
            avg_mc_weighted = float(args.stage2_mc_weight) * avg_mc
            avg_obj_weighted = float(args.stage2_obj_weight) * avg_obj
            avg_reg_weighted = float(args.stage2_reg_weight) * avg_reg
            avg_answer_anchor_weighted = (
                float(args.stage2_answer_anchor_weight) * avg_answer_anchor
            )
            avg_wrong_weighted = float(args.stage2_wrong_image_weight) * avg_wrong
            period_counter += 1
            if is_main_process():
                print(
            ...
            if (
                ...
                if is_main_process():
                    print(
                        ...
                        f"mode={stage2_eval_answer_mode}, schema={readable_schema}, n={val_n}"
                    )
                    if tb_writer is not None:
                        tb_writer.add_scalar("stage2_eval/val_answer_acc", val_acc, period_counter)
                        tb_writer.add_scalar("stage2_eval/format_rate", val_fmt, period_counter)
                if is_distributed():
                    barrier_if_distributed()
