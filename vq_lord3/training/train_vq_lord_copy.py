...
from vq_lord3.models.vq_module import VQVisionEncoder
from vq_lord3.data.data_collector import (
    GPT4VDataCollector,
    VQLORDDataset,
    VisualQAItem,
...
class ScienceQADataset(torch.utils.data.Dataset):
    """ScienceQA 多模态数据集包装（兼容 Stage1 双监督与 Stage2 单监督）。"""
    ...
    def __init__(
        ...
        stage2_vic_include_context: bool = False,
        ...
        self.stage2_vic_include_context = bool(stage2_vic_include_context)
    ...
    def _build_targets(self, item: dict, instruction_text: str) -> tuple[str, str, str, bool]:
        ...
        # 固定答案锚点，确保 Stage1/Stage2 指标口径一致。
        ...
        if not isinstance(ann, dict):
            if self.require_teacher_annotation:
                ...
                raise RuntimeError(
                    ...
                    "Stage1/3 训练要求四字段教师标注全覆盖。"
                )
            # Stage0 或未启用教师采集时，允许回退到旧 response（仅用于保持训练流程可跑）。
        ...
        if self.stage2_vic_include_context:
...
def save_stage2_checkpoint(model, save_dir: str):
    ...
    print(f"Stage2 检查点已保存至 {save_dir}")
...
def setup_args():
    ...
    parser.add_argument("--lr", type=float, default=3e-5,
                       help="学习率")
    parser.add_argument("--stage0_lr", type=float, default=0.0,
                       help="Stage0 学习率，<=0 时使用 lr*5")
    parser.add_argument("--stage0_recon_weight", type=float, default=1.0,
                       help="Stage0 特征重建损失权重")
    parser.add_argument("--stage0_cosine_weight", type=float, default=0.25,
                       help="Stage0 特征余弦损失权重")
    parser.add_argument("--stage0_vq_weight", type=float, default=1.0,
                       help="Stage0 VQ 损失权重")
    parser.add_argument("--stage0_grad_clip", type=float, default=5.0,
                       help="Stage0 梯度裁剪阈值；<=0 关闭")
    ...
    parser.add_argument("--stage1_grad_accum", type=int, default=0,
                       help="Stage1 梯度累积步数，0 表示回退到 grad_accum")
    parser.add_argument("--stage1_answer_weight", type=float, default=1.0,
                       help="Stage1 答案监督权重")
    parser.add_argument("--stage1_rationale_weight", type=float, default=0.3,
                       help="Stage1 解释监督权重")
    parser.add_argument("--stage1_prepost_lr_scale", type=float, default=0.5,
                       help="Stage1 pre/post quant 学习率缩放")
    parser.add_argument("--stage1_vision_lr_scale", type=float, default=0.2,
                       help="Stage1 视觉塔学习率缩放")
    ...
    parser.add_argument("--stage2_lr_scale", type=float, default=0.2,
                       help="Stage2 学习率缩放，默认低于 Stage1")
    parser.add_argument("--stage2_grad_clip", type=float, default=1.0,
                       help="Stage2 梯度裁剪阈值；<=0 表示关闭")
    parser.add_argument("--stage2_train_projector", type=int, default=0,
                       help="Stage2 是否继续训练 projector，默认冻结以保护 Stage1 建立的视觉桥")
    parser.add_argument("--sub_stage_num", type=int, default=1,
                       help="Stage2 外层 sub-stage 数")
    parser.add_argument("--period_num", type=int, default=0,
                       help="Stage2 每个 sub-stage 的 period 数；<=0 时回退到 epochs")
    parser.add_argument("--sub_set_num", type=int, default=0,
                       help="Stage2 每个 sub-stage 的样本子集大小；<=0 表示全量")
    parser.add_argument("--stage2_eval_max_samples", type=int, default=0,
                       help="Stage2 每个 period 评估样本数；<=0 关闭 period 评估")
    parser.add_argument("--stage2_eval_every_period", type=int, default=1,
                       help="Stage2 每隔多少个 period 做一次评估")
    parser.add_argument("--stage2_eval_scienceqa_split", type=str, default="validation",
                       help="Stage2 独立评估使用的 ScienceQA split（默认 validation）")
    parser.add_argument("--stage2_eval_scienceqa_path", type=str, default="",
                       help="Stage2 独立评估使用的数据集路径（为空则复用 scienceqa_path）")
    parser.add_argument("--stage2_eval_train_num", type=int, default=0,
                       help="Stage2 独立评估样本数上限（0=该 split 全量）")
    parser.add_argument("--stage2_eval_answer_mode", type=str, default="logits",
                       choices=["generate", "logits"],
                       help="Stage2 内置评估口径：generate 或 logits；准确率优先时建议 logits")
    parser.add_argument("--stage2_field_weight_observed", type=float, default=1.0,
                       help="Stage2 教师正则中 observed_facts_visual 的 token 权重")
    parser.add_argument("--stage2_field_weight_context", type=float, default=1.0,
                       help="Stage2 教师正则中 context_textual 的 token 权重")
    parser.add_argument("--stage2_field_weight_reasoning", type=float, default=1.0,
                       help="Stage2 教师正则中 reasoning 的 token 权重")
    parser.add_argument("--stage2_field_weight_answer", type=float, default=1.0,
                       help="Stage2 教师正则中 answer 的 token 权重")
    parser.add_argument("--stage2_obj_weight", type=float, default=1.0,
                       help="Stage2 偏好目标 L_obj 的总权重")
    parser.add_argument("--stage2_reg_weight", type=float, default=1.0,
                       help="Stage2 教师正则 L_reg 的总权重")
    parser.add_argument("--stage2_answer_anchor_weight", type=float, default=1.0,
                       help="Stage2 answer token 直接锚定损失权重（基于 y_vic 的 Answer: X token NLL）")
    parser.add_argument("--stage2_mc_weight", type=float, default=1.0,
                       help="Stage2 选择题主损失权重（基于 Answer: 后下一 token logits 的多选 CE）")
    parser.add_argument("--stage2_pair_use_answer_correctness", type=int, default=1,
                       help="Stage2 Phase-A 是否优先按答案正确性选择 y+/y-，1=启用，0=关闭")
    parser.add_argument("--stage2_wrong_image_enable", type=int, default=0,
                       help="Stage2 是否启用错图负样本约束，默认关闭")
    parser.add_argument("--stage2_wrong_image_weight", type=float, default=0.2,
                       help="Stage2 错图负样本损失权重")
    parser.add_argument("--stage2_wrong_image_margin", type=float, default=0.0,
                       help="Stage2 错图负样本对比边际（token-level log-prob）")
    parser.add_argument("--stage2_force_cold_start_period0", type=int, default=1,
                       help="Stage2 是否在 period0 强制使用 y_vic 作为正样本，1=启用，0=关闭")
    ...
    parser.add_argument("--stage2_vic_include_context", type=int, default=0,
                       help="Stage2 的 y_vic 是否包含 context_textual（默认0，仅 facts+reasoning+answer）")
    parser.add_argument("--reuse_vq_codebook", type=int, default=1,
                       help="若存在已保存的VQ codebook则复用并跳过Stage0")
    parser.add_argument("--vq_codebook_path", type=str, default="",
                       help="VQ codebook 保存/加载路径 (为空则使用save_path/stage0_vq)")
    parser.add_argument("--reuse_stage1", type=int, default=1,
                       help="若存在已保存的Stage1模型则复用并跳过Stage1")
    parser.add_argument("--stage1_ckpt_path", type=str, default="",
                       help="Stage1 checkpoint 保存/加载路径 (为空则使用save_path/stage1_vision)")
    parser.add_argument("--scienceqa_preprocessed_path", type=str, default="",
                       help="ScienceQA 预处理结果 JSON 路径，供 Stage0/2 分桶采样使用")
    parser.add_argument("--bucket_batch_size", type=int, default=0,
                       help="分桶采样 batch size 覆盖值，0 表示使用预处理 JSON 中的 batch size")
    parser.add_argument("--stage2_bucket_batch_size", type=int, default=0,
                       help="Stage2 分桶采样 batch size，0 表示沿用 bucket_batch_size 或预处理 JSON")
    parser.add_argument("--disable_bucket_for_stage2", type=int, default=1,
                       help="是否在 Stage2 禁用分桶采样，1=禁用，0=启用")
    ...
    parser.add_argument("--save_each_epoch", type=int, default=0,
                       help="是否在 Stage0/2/3 每个 epoch 结束时额外保存一次检查点")
    parser.add_argument("--stage2_resume_path", type=str, default="",
                       help="Stage2 断点目录，非空时从该目录恢复（包含模型可训练参数/optimizer/RNG/period状态）")
    ...
    parser.add_argument("--stage2_resume_save_optimizer", type=int, default=1,
                       help="Stage2 断点是否保存 optimizer state（1=保存,0=不保存，文件更小）")
    parser.add_argument("--stage2_resume_save_interval", type=int, default=1,
                       help="Stage2 每隔多少个 period 保存一次断点")
    parser.add_argument("--stage2_sample_cache_path", type=str, default="",
                       help="Stage2SampleCache 落盘路径；命中且元信息匹配时直接复用")
...
def train_stage0_vq(model, dataloader, args, tb_writer):
    """
    ...
    for param in vq_encoder.stage0_parameters():
    ...
    stage0_lr = args.stage0_lr if args.stage0_lr > 0 else args.lr * 5
    print(
        f"[Stage0] lr={stage0_lr}, recon_w={args.stage0_recon_weight}, "
        f"cos_w={args.stage0_cosine_weight}, vq_w={args.stage0_vq_weight}, "
        f"grad_clip={args.stage0_grad_clip}"
    )

    # 对齐 taming VQGAN：Stage0 使用 Adam(无 weight decay)，避免 codebook 被 AdamW 拉向 0。
    ...
    for epoch in range(args.epochs):
        ...
        for batch in tqdm(dataloader, desc=f"VQ预训练 Epoch {epoch+1}"):
            ...
            stage0_out = vq_encoder.stage0_forward(pixel_values)

            target_features = stage0_out["target_features"]
            reconstructed_features = stage0_out["reconstructed_features"]
            vq_loss = stage0_out["vq_loss"]
            vq_indices = stage0_out.get("indices")
            ...
            total_loss = (
                args.stage0_recon_weight * recon_loss
                + args.stage0_cosine_weight * cosine_loss
                + args.stage0_vq_weight * vq_loss
            )

            if not torch.isfinite(total_loss):
                print(
                    f"[Stage0][Warn] step={global_step} 出现非有限损失，"
            ...
            total_loss.backward()
            grad_norm = None
            if args.stage0_grad_clip > 0:
                grad_norm = torch.nn.utils.clip_grad_norm_([
                    p for p in model.parameters() if p.requires_grad
                ], max_norm=args.stage0_grad_clip)
                if isinstance(grad_norm, torch.Tensor) and not torch.isfinite(grad_norm):
                    print(f"[Stage0][Warn] step={global_step} 梯度范数非有限，跳过 optimizer.step()")
                    optimizer.zero_grad(set_to_none=True)
                    continue
            optimizer.step()
            ...
            if global_step % args.log_step == 0:
                ...
                tb_writer.add_scalar("stage0/total_loss", total_loss_val, global_step)
                tb_writer.add_scalar("stage0/recon_loss", recon_loss_val, global_step)
                tb_writer.add_scalar("stage0/cosine_loss", cosine_loss_val, global_step)
                tb_writer.add_scalar("stage0/vq_loss", vq_loss_val, global_step)
                tb_writer.add_scalar("stage0/loss_ema", loss_ema, global_step)
                tb_writer.add_scalar("stage0/codebook_used", codebook_used, global_step)
                tb_writer.add_scalar("stage0/perplexity", perplexity_val, global_step)
                tb_writer.add_scalar("stage0/dead_code_resets", dead_code_resets, global_step)
                tb_writer.add_scalar("stage0/dead_code_count", dead_code_count, global_step)
                with open(metrics_path, "a", encoding="utf-8") as f:
                    f.write(
                        f"stage0,{global_step},{total_loss_val:.6f},{recon_loss_val:.6f},"
        ...
        if int(getattr(args, "save_each_epoch", 0)) == 1:
            epoch_dir = os.path.join(args.save_path, f"stage0_vq_epoch{epoch+1}")
            epoch_codebook_path = os.path.join(epoch_dir, "vq_codebook.pt")
            save_vq_codebook(model, epoch_codebook_path)
            print(f"Stage0 Epoch 检查点已保存至 {epoch_dir}")
...
def train_stage1_vision(model, dataloader, args, tb_writer):
    ...
    print(
        ...
        f"answer_w={args.stage1_answer_weight}, rationale_w={args.stage1_rationale_weight}, "
    ...
    for epoch in range(args.epochs):
        ...
        for batch_idx, batch in enumerate(tqdm(dataloader, desc=f"视觉蒸馏 Epoch {epoch+1}")):
            ...
            answer_vq_loss = _get_stage1_vq_loss(model, args.device, answer_loss.dtype)
            ...
            if has_any_rationale:
                ...
                vq_loss = _get_stage1_vq_loss(model, args.device, answer_loss.dtype)
            else:
                rationale_loss = torch.zeros((), device=args.device, dtype=answer_loss.dtype)
                vq_loss = answer_vq_loss

            total_loss = (
                float(args.stage1_answer_weight) * answer_loss
                + float(args.stage1_rationale_weight) * rationale_loss
                + args.beta * vq_loss
            )

            if not torch.isfinite(total_loss):
                print(
                    f"[Stage1][Warn] step={global_step} 非有限损失，跳过。"
                )
                optimizer.zero_grad(set_to_none=True)
                continue
            ...
            if global_step % args.log_step == 0:
                ...
                tb_writer.add_scalar("stage1/answer_loss", answer_loss_val, global_step)
                tb_writer.add_scalar("stage1/rationale_loss", rationale_loss_val, global_step)
                tb_writer.add_scalar("stage1/vq_loss", vq_loss_val, global_step)
                tb_writer.add_scalar("stage1/vq_answer_ratio", weighted_vq_ratio, global_step)
                ...
                tb_writer.add_scalar("stage1/answer_only_accuracy_proxy", answer_acc_proxy, global_step)
        ...
        tb_writer.add_scalar("stage1_epoch/total_loss", epoch_total / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/answer_loss", epoch_answer / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/rationale_loss", epoch_rationale / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/vq_loss", epoch_vq / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/vq_answer_ratio", epoch_vq_ratio / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/vq_perplexity", epoch_vq_perplexity / denom, epoch + 1)
        tb_writer.add_scalar("stage1_epoch/answer_only_accuracy_proxy", epoch_acc / denom, epoch + 1)
        if int(getattr(args, "save_each_epoch", 0)) == 1:
...
def _save_stage2_resume_state(
    ...
    torch.save(payload, os.path.join(resume_dir, "stage2_resume_state.pt"))
    meta = dict(progress)
    ...
    with open(os.path.join(resume_dir, "stage2_resume_meta.json"), "w") as f:
...
def _load_stage2_resume_state(
    ...
    state_path = os.path.join(resume_dir, "stage2_resume_state.pt")
    if not os.path.exists(state_path):
        print(f"[Stage2][Resume] 未找到断点文件: {state_path}")
        return None
    ...
    print(
        f"[Stage2][Resume] 已恢复进度: next_sub_stage={progress.get('next_sub_stage_idx', 0)}, "
...
class PeriodTrainingDataset(torch.utils.data.Dataset):
    def __init__(self, items: List[PeriodTrainingItem], sample_cache: List[Stage2SampleCacheItem]):
        self.items = items
...
def _build_stage2_sample_cache(train_dataset) -> List[Stage2SampleCacheItem]:
    cache: List[Stage2SampleCacheItem] = []
    processor = getattr(train_dataset, "processor", None)
    tokenizer = getattr(getattr(train_dataset, "processor", None), "tokenizer", None)
    for idx in tqdm(range(len(train_dataset)), desc="构建 Stage2SampleCache"):
        ...
        # Stage2 y_vic 默认走结构化组合（facts+reasoning+answer，context 可由开关控制）。
        if (
            y_vic_ids is None
        ...
        cache.append(Stage2SampleCacheItem(
            sample_idx=int(idx),
            ...
            answer_idx=int(sample.get("answer_idx", raw_item.get("answer_idx", -1)) if isinstance(raw_item, dict) else sample.get("answer_idx", -1)),
            num_choices=len(raw_item.get("choices", [])) if isinstance(raw_item, dict) else 0,
        ))
    return cache


def _resolve_stage2_sample_cache_path(args) -> str:
    cache_path = str(getattr(args, "stage2_sample_cache_path", "") or "").strip()
    if cache_path:
        return cache_path
    return os.path.join(str(getattr(args, "save_path", ".") or "."), "stage2_sample_cache.pt")


def _resolve_stage2_eval_sample_cache_path(
    base_cache_path: str,
    eval_split: str,
    eval_path: str,
    eval_train_num: int,
...
def _load_stage2_sample_cache(cache_path: str) -> Optional[List[Stage2SampleCacheItem]]:
    if not cache_path or not os.path.exists(cache_path):
        return None
    try:
        payload = torch.load(cache_path, map_location="cpu")
    except Exception as exc:
        print(f"[Stage2][Cache][Warn] 读取 sample cache 失败，将重建: {cache_path} ({exc})")
        return None
    ...
    if isinstance(payload, list):
        ...
        print(f"[Stage2][Cache][Warn] sample cache 格式无效，将重建: {cache_path}")
        return None
    if cache_items:
        probe = cache_items[0]
        missing_fields = []
        ...
        if missing_fields:
            print(
                f"[Stage2][Cache][Warn] sample cache 缺少字段 {missing_fields}，将重建: {cache_path}"
            )
            return None
    print(f"[Stage2][Cache] 已复用 Stage2SampleCache: {cache_path}, samples={len(cache_items)}")
    return cache_items


def _save_stage2_sample_cache(cache_path: str, cache_items: List[Stage2SampleCacheItem]):
    if not cache_path:
        return
    try:
        ...
        print(f"[Stage2][Cache] 已保存 Stage2SampleCache: {cache_path}, samples={len(cache_items)}")
    except Exception as exc:
        print(f"[Stage2][Cache][Warn] 保存 sample cache 失败，忽略: {cache_path} ({exc})")


def _set_stage2_trainable_params(model, args) -> List[torch.nn.Parameter]:
    for name, param in model.named_parameters():
        if not torch.is_floating_point(param):
            param.requires_grad = False
            continue
        ...
        is_projector = (
            ...
            and bool(args.stage2_train_projector)
        )
...
def _generate_candidate_batch(
    model,
    samples: List[Stage2SampleCacheItem],
    args,
    image_token_id: Optional[int],
    pad_token_id: int,
    do_sample: bool = True,
...
def _build_stage2_choice_token_map(tokenizer, max_choices: int = 8) -> Dict[int, List[int]]:
...
def _evaluate_stage2_answer_metrics(
    model,
    sample_cache: List[Stage2SampleCacheItem],
    ...
    eval_mode = str(getattr(args, "stage2_eval_answer_mode", "logits") or "logits").strip().lower()
    model.eval()
    ...
    try:
        ...
        if eval_mode == "logits" and choice_token_map is not None:
            ...
            sorted_indices = sorted(eval_indices, key=lambda idx: _stage2_static_sort_key(sample_cache[idx]))
...
def _get_stage2_phase_a_batch_size(args, train_bucket_meta: Optional[dict] = None) -> int:
    batch_size = max(1, int(getattr(args, "batch_size", 1)))
    ...
    if int(getattr(args, "bucket_batch_size", 0)) > 0:
        batch_size = int(getattr(args, "bucket_batch_size", 0))
    if int(getattr(args, "stage2_bucket_batch_size", 0)) > 0:
        batch_size = int(getattr(args, "stage2_bucket_batch_size", 0))
    return max(1, batch_size)


def _stage2_static_sort_key(sample: Stage2SampleCacheItem) -> Tuple[int, int, int, int]:
...
def _build_stage2_static_batches(
    ...
    batch_size = _get_stage2_phase_a_batch_size(args, train_bucket_meta)
    if batch_size <= 1:
        return [[int(idx)] for idx in active_indices]

    use_bucket = (
        getattr(args, "dataset_name", "") == "scienceqa"
        and train_bucket_meta is not None
        and not bool(int(getattr(args, "disable_bucket_for_stage2", 1)))
    )
    ...
    if use_bucket:
        ...
        for idx in active_indices:
            bucket_key = sample_to_bucket.get(int(idx), "__stage2_fallback__")
        ...
        for bucket_key in sorted(bucket_to_indices.keys()):
            bucket_indices = sorted(
                ...
                key=lambda i: _stage2_static_sort_key(sample_cache[i]),
            )
            for start in range(0, len(bucket_indices), batch_size):
                batches.append(bucket_indices[start:start + batch_size])
        return batches

    sorted_indices = sorted(active_indices, key=lambda i: _stage2_static_sort_key(sample_cache[i]))
    for start in range(0, len(sorted_indices), batch_size):
...
def _bootstrap_period_states(
    ...
    gc_states = _set_model_gradient_checkpointing(model, False)
    try:
        stage2_batches = _build_stage2_static_batches(
            active_indices=active_indices,
            sample_cache=sample_cache,
            args=args,
            train_bucket_meta=train_bucket_meta,
        )
        for batch_indices in tqdm(stage2_batches, desc="Stage2 bootstrap"):
            batch_samples = [sample_cache[idx] for idx in batch_indices]
...
def _build_pairs_and_next_states(
    ...
    try:
        ...
        for batch_indices in tqdm(stage2_batches, desc="Stage2 Phase-A"):
            batch_samples = [sample_cache[idx] for idx in batch_indices]
            ...
            for row_idx, idx in enumerate(batch_indices):
                ...
                if bool(int(getattr(args, "stage2_pair_use_answer_correctness", 1))) and tokenizer is not None:
...
def _run_one_period_train(
    ...
    grad_clip = float(getattr(args, "stage2_grad_clip", 1.0))
    wrong_image_enable = bool(int(getattr(args, "stage2_wrong_image_enable", 0)))
    wrong_image_weight = float(getattr(args, "stage2_wrong_image_weight", 0.2))
    wrong_image_margin = float(getattr(args, "stage2_wrong_image_margin", 0.0))
    obj_weight = float(getattr(args, "stage2_obj_weight", 1.0))
    reg_weight = float(getattr(args, "stage2_reg_weight", 1.0))
    answer_anchor_weight = float(getattr(args, "stage2_answer_anchor_weight", 1.0))
    mc_weight = float(getattr(args, "stage2_mc_weight", 1.0))
    field_weights = {
        "observed": float(getattr(args, "stage2_field_weight_observed", 1.0)),
        "context": float(getattr(args, "stage2_field_weight_context", 1.0)),
        "reasoning": float(getattr(args, "stage2_field_weight_reasoning", 1.0)),
        "answer": float(getattr(args, "stage2_field_weight_answer", 1.0)),
    }
    optimizer.zero_grad(set_to_none=True)
    ...
    for batch_idx, batch in enumerate(tqdm(loader, desc=f"Stage2 S{sub_stage_idx+1} P{period_idx+1}")):
        if batch is None:
            continue
        global_step += 1
        steps += 1
        ...
        if global_step % args.log_step == 0:
            ...
            tb_writer.add_scalar("stage2/total_loss", loss_val, global_step)
            tb_writer.add_scalar("stage2/L_mc", mc_val, global_step)
            tb_writer.add_scalar("stage2/L_mc_weighted", mc_weight * mc_val, global_step)
            tb_writer.add_scalar("stage2/mc_acc_proxy", mc_acc_proxy, global_step)
            tb_writer.add_scalar("stage2/L_obj", obj_val, global_step)
            tb_writer.add_scalar("stage2/L_reg", reg_val, global_step)
            tb_writer.add_scalar("stage2/L_obj_weighted", obj_weight * obj_val, global_step)
            tb_writer.add_scalar("stage2/L_reg_weighted", reg_weight * reg_val, global_step)
            tb_writer.add_scalar("stage2/L_answer_anchor", answer_anchor_val, global_step)
            tb_writer.add_scalar("stage2/L_wrong_image", wrong_val, global_step)
            tb_writer.add_scalar("stage2/L_reg_observed", field_loss_vals["observed"], global_step)
            tb_writer.add_scalar("stage2/L_reg_context", field_loss_vals["context"], global_step)
            tb_writer.add_scalar("stage2/L_reg_reasoning", field_loss_vals["reasoning"], global_step)
            tb_writer.add_scalar("stage2/L_reg_answer", field_loss_vals["answer"], global_step)
...
def train_stage2_lord(model, train_dataset, args, tb_writer, train_bucket_meta: Optional[dict] = None):
    print("\n" + "=" * 50)
    ...
    stage2_lr = float(args.lr) * float(getattr(args, "stage2_lr_scale", 1.0))
    stage2_phase_a_batch_size = _get_stage2_phase_a_batch_size(args, train_bucket_meta)
    stage2_use_static_bucket = bool(
        getattr(args, "dataset_name", "") == "scienceqa"
        and train_bucket_meta is not None
        and not bool(int(getattr(args, "disable_bucket_for_stage2", 1)))
    )
    stage2_eval_answer_mode = str(getattr(args, "stage2_eval_answer_mode", "logits") or "logits").strip().lower()

    print(
        f"[Stage2] tau1={tau1}, tau_delta={tau_delta}, "
        f"sub_stage_num={sub_stage_num}, period_num={period_num}, sub_set_num={sub_set_num}, "
        f"lr={stage2_lr}, train_projector={int(bool(args.stage2_train_projector))}, "
        f"resume_opt={int(bool(getattr(args, 'stage2_resume_save_optimizer', 1)))}, "
        f"eval_max_samples={int(getattr(args, 'stage2_eval_max_samples', 0))}, "
        f"field_w=({float(getattr(args, 'stage2_field_weight_observed', 1.0)):.2f},"
        f"{float(getattr(args, 'stage2_field_weight_context', 1.0)):.2f},"
        f"{float(getattr(args, 'stage2_field_weight_reasoning', 1.0)):.2f},"
        f"{float(getattr(args, 'stage2_field_weight_answer', 1.0)):.2f}), "
        f"mc_w={float(getattr(args, 'stage2_mc_weight', 1.0)):.2f}, "
        f"obj_w={float(getattr(args, 'stage2_obj_weight', 1.0)):.2f}, "
        f"reg_w={float(getattr(args, 'stage2_reg_weight', 1.0)):.2f}, "
        f"ans_anchor_w={float(getattr(args, 'stage2_answer_anchor_weight', 1.0)):.2f}, "
        f"pair_by_answer={int(bool(getattr(args, 'stage2_pair_use_answer_correctness', 1)))}, "
        f"wrong_image={int(bool(getattr(args, 'stage2_wrong_image_enable', 0)))}, "
        f"force_cold_start_p0={int(bool(getattr(args, 'stage2_force_cold_start_period0', 1)))}, "
        f"phaseA_batch_size={stage2_phase_a_batch_size}, static_bucket={int(stage2_use_static_bucket)}, "
        f"eval_mode={stage2_eval_answer_mode}"
    )

    trainable_params = _set_stage2_trainable_params(model, args)
    print(f"Stage2 可训练参数量: {sum(p.numel() for p in trainable_params):,}")
    if not trainable_params:
        raise RuntimeError("Stage2 没有可训练参数，请检查 LoRA 或 stage2_train_projector 配置")
    optimizer = torch.optim.AdamW(trainable_params, lr=stage2_lr)

    stage2_sample_cache_path = _resolve_stage2_sample_cache_path(args)
    sample_cache = _load_stage2_sample_cache(stage2_sample_cache_path)
    if sample_cache is None:
        sample_cache = _build_stage2_sample_cache(train_dataset)
        _save_stage2_sample_cache(stage2_sample_cache_path, sample_cache)
    if not sample_cache:
        raise RuntimeError("Stage2SampleCache 为空，无法开始 Stage2 训练")
    ...
    if tokenizer is not None:
        choice_token_map = _build_stage2_choice_token_map(tokenizer, max_choices=26)
    ...
    stage2_eval_max_samples = int(getattr(args, "stage2_eval_max_samples", 0))
    eval_sample_cache: Optional[List[Stage2SampleCacheItem]] = None
    eval_answer_lookup: Dict[int, str] = {}
    eval_pool_indices: Optional[List[int]] = None
    stage2_eval_split = str(
        getattr(args, "stage2_eval_scienceqa_split", "validation") or ""
    ).strip()
    stage2_eval_path = str(
        getattr(args, "stage2_eval_scienceqa_path", "") or getattr(args, "scienceqa_path", "")
    ).strip()
    stage2_eval_train_num = int(getattr(args, "stage2_eval_train_num", 0))
    stage2_eval_sample_cache_path = _resolve_stage2_eval_sample_cache_path(
        stage2_sample_cache_path,
        stage2_eval_split,
        stage2_eval_path,
        stage2_eval_train_num,
    )
    if (
        stage2_eval_max_samples > 0
        and tokenizer is not None
        and stage2_eval_split
        and stage2_eval_path
        and hasattr(train_dataset, "processor")
    ):
        same_eval_and_train = (
            stage2_eval_split == str(getattr(args, "scienceqa_split", "")).strip()
            and stage2_eval_path == str(getattr(args, "scienceqa_path", "")).strip()
        )
        if same_eval_and_train:
            print(
                "[Stage2][Warn] stage2_eval_scienceqa_split 与训练 split 相同，"
                "当前评估将与训练集重合。建议使用 validation。"
            )
        try:
            eval_samples = build_scienceqa_samples(
                scienceqa_path=stage2_eval_path,
                split=stage2_eval_split,
                train_num=stage2_eval_train_num,
            ...
            eval_dataset = ScienceQADataset(
                ...
                stage2_vic_include_context=bool(int(args.stage2_vic_include_context)),
                require_teacher_annotation=False,
            )
            eval_sample_cache = _load_stage2_sample_cache(stage2_eval_sample_cache_path)
            if eval_sample_cache is None:
                eval_sample_cache = _build_stage2_sample_cache(eval_dataset)
                _save_stage2_sample_cache(stage2_eval_sample_cache_path, eval_sample_cache)
            ...
            print(
                f"[Stage2][Eval] 使用独立评估集: split={stage2_eval_split}, "
                f"path={stage2_eval_path}, samples={len(eval_sample_cache)}"
            )
        except Exception as exc:
            print(f"[Stage2][Warn] 构建独立评估集失败，将回退训练集评估: {exc}")
    ...
    stage2_eval_every_period = max(1, int(getattr(args, "stage2_eval_every_period", 1)))
    resume_save_dir = str(getattr(args, "stage2_resume_save_path", "") or "").strip()
    if not resume_save_dir:
        resume_save_dir = os.path.join(args.save_path, "stage2_resume_latest")

    resume_path = str(getattr(args, "stage2_resume_path", "") or "").strip()
    resume_progress = None
    if resume_path:
        resume_progress = _load_stage2_resume_state(model, optimizer, resume_path, args.device)
    ...
    if resume_sub_stage_idx >= sub_stage_num:
        print("[Stage2][Resume] next_sub_stage_idx 超出配置，Stage2 无需继续训练。")
        return model
    ...
    for sub_stage_idx in range(resume_sub_stage_idx, sub_stage_num):
        ...
        print(
            f"[Stage2] sub_stage={sub_stage_idx+1}/{sub_stage_num}, "
        ...
        if is_resume_sub_stage and resume_period_idx > 0 and resume_period_states is not None:
            ...
            print(
                f"[Stage2][Resume] 继续 sub_stage={sub_stage_idx+1}, period={resume_period_idx+1}/{period_num}"
            )
        else:
        ...
        for period_idx in range(resume_period_idx, period_num):
            print(f"[Stage2] 进入 period {period_idx+1}/{period_num}")
            ...
            training_items, next_states, cold_start_count, swap_count, plus_lp_sum, minus_lp_sum = _build_pairs_and_next_states(
                ...
                force_vic_positive=bool(
                    period_idx == 0 and int(getattr(args, "stage2_force_cold_start_period0", 1))
                ),
                train_bucket_meta=train_bucket_meta,
                tokenizer=tokenizer,
            )
            ...
            avg_mc_weighted = float(getattr(args, "stage2_mc_weight", 1.0)) * avg_mc
            avg_obj_weighted = float(getattr(args, "stage2_obj_weight", 1.0)) * avg_obj
            avg_reg_weighted = float(getattr(args, "stage2_reg_weight", 1.0)) * avg_reg
            avg_answer_anchor_weighted = (
                float(getattr(args, "stage2_answer_anchor_weight", 1.0)) * avg_answer_anchor
            )
            avg_wrong_weighted = float(getattr(args, "stage2_wrong_image_weight", 0.2)) * avg_wrong
            period_counter += 1
            print(
                f"[Stage2][S{sub_stage_idx+1}P{period_idx+1}] "
            ...
            tb_writer.add_scalar("stage2_epoch/total_loss", avg_total, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_mc", avg_mc, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_mc_weighted", avg_mc_weighted, period_counter)
            tb_writer.add_scalar("stage2_epoch/mc_acc_proxy", avg_mc_acc, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_obj", avg_obj, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg", avg_reg, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_obj_weighted", avg_obj_weighted, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg_weighted", avg_reg_weighted, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_answer_anchor", avg_answer_anchor, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_answer_anchor_weighted", avg_answer_anchor_weighted, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_wrong_image", avg_wrong, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_wrong_image_weighted", avg_wrong_weighted, period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg_observed", avg_field_losses["observed"], period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg_context", avg_field_losses["context"], period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg_reasoning", avg_field_losses["reasoning"], period_counter)
            tb_writer.add_scalar("stage2_epoch/L_reg_answer", avg_field_losses["answer"], period_counter)
            tb_writer.add_scalar("stage2/cold_start_ratio", cold_ratio, period_counter)
            tb_writer.add_scalar("stage2/swap_ratio", swap_ratio, period_counter)
            tb_writer.add_scalar("stage2/avg_lp_plus", avg_lp_plus, period_counter)
            tb_writer.add_scalar("stage2/avg_lp_minus", avg_lp_minus, period_counter)
            tb_writer.add_scalar("stage2/phase_a_seconds", phase_a_seconds, period_counter)
            tb_writer.add_scalar("stage2/period", period_idx + 1, period_counter)
            tb_writer.add_scalar("stage2/sub_stage", sub_stage_idx + 1, period_counter)

            if (
                stage2_eval_max_samples > 0
                and tokenizer is not None
                and (answer_lookup or eval_answer_lookup)
                and (period_counter % stage2_eval_every_period == 0)
            ):
                use_eval_cache = (
                ...
                eval_k = min(stage2_eval_max_samples, len(metrics_indices_pool))
                ...
                val_acc, val_fmt, val_n = _evaluate_stage2_answer_metrics(
                    model=model,
                ...
                print(
                    f"[Stage2][Eval][S{sub_stage_idx+1}P{period_idx+1}] "
                    f"val_answer_acc={val_acc:.4f}, format_rate={val_fmt:.4f}, "
                    f"mode={stage2_eval_answer_mode}, n={val_n}"
                )
                tb_writer.add_scalar("stage2/val_answer_acc", val_acc, period_counter)
                tb_writer.add_scalar("stage2/format_rate", val_fmt, period_counter)
            ...
            if int(getattr(args, "save_each_epoch", 0)) == 1:
                ckpt_dir = os.path.join(
                    args.save_path,
                    f"stage2_sub{sub_stage_idx+1}_period{period_idx+1}",
                )
                save_stage2_checkpoint(model, ckpt_dir)
...
def save_stage0_checkpoint(model, args):
    """Stage0 只保存后续蒸馏真正需要的 VQ stack，避免整模型落盘占用大量磁盘。"""
    save_dir = os.path.join(args.save_path, "stage0_vq")
    os.makedirs(save_dir, exist_ok=True)
    save_vq_codebook(model, args.vq_codebook_path)
    print(f"Stage0 VQ stack 已保存至 {save_dir}")
...
def save_stage1_checkpoint(model, args, ckpt_path: str):
    """保存 Stage1 检查点（包括 VQ codebook 和 LoRA 权重）"""
    ...
    config_info = {
        ...
        "stage1_train_lora": 1,
        "stage1_train_codebook": 0,
        "stage1_answer_format": "letter",
        "projector_state_path": "projector.pt",
        "projector_state_param_count": int(projector_param_count),
    }
    with open(os.path.join(ckpt_path, "stage1_config.json"), "w") as f:
        json.dump(config_info, f, indent=2)
    
    print(f"Stage1 检查点已保存至 {ckpt_path}")


def load_stage1_checkpoint(model, ckpt_path: str):
    """加载 Stage1 检查点"""
    ...
    if os.path.exists(adapter_config):
        ...
        # 直接覆盖现有 default adapter，避免额外 adapter name 干扰 Stage2 参数遍历。
    ...
    config_path = os.path.join(ckpt_path, "stage1_config.json")
    if os.path.exists(config_path):
        with open(config_path, "r", encoding="utf-8") as f:
            stage1_cfg = json.load(f)
        print(
            f"已读取 Stage1 配置: stage={stage1_cfg.get('stage')}, "
            f"answer_w={stage1_cfg.get('stage1_answer_weight')}, "
            f"rationale_w={stage1_cfg.get('stage1_rationale_weight')}"
        )
    else:
        print(f"[Warn] 未找到 stage1_config.json: {ckpt_path}")
    
    print(f"Stage1 检查点加载完成")
    return model


def _stage1_contract_missing_files(ckpt_path: str) -> List[str]:
    required_files = [
        "vq_codebook.pt",
        "projector.pt",
        "stage1_config.json",
...
def main():
    ...
    if args.reuse_vq_codebook:
        print("[Info] reuse_vq_codebook=1，若存在 stage0 codebook 将跳过 Stage0。")
    if args.reuse_stage1:
        print("[Info] reuse_stage1=1，若存在 stage1 ckpt 将跳过 Stage1。")
    
    # 创建保存目录
    os.makedirs(args.save_path, exist_ok=True)
    if not args.vq_codebook_path:
        args.vq_codebook_path = os.path.join(args.save_path, "stage0_vq", "vq_codebook.pt")
    tb_writer = SummaryWriter(log_dir=os.path.join(args.save_path, "logs"))

    # 先准备 ScienceQA 数据；仅 stage>=1 时再准备教师回答，避免 Stage0-only 多余 API 调用。
    train_samples = None
    ...
    if args.dataset_name == "scienceqa":
        ...
        if int(args.stage) >= 1:
            # 关键：仅在 Stage1/3 需要蒸馏时补齐教师回答
            print("加载 ScienceQA 教师回答（stage>=1）...")
    ...
    if args.dataset_name == "scienceqa":
        train_dataset = ScienceQADataset(
            ...
            teacher_answer_max_tokens=args.teacher_answer_max_tokens,
            stage2_vic_include_context=bool(int(args.stage2_vic_include_context)),
            require_teacher_annotation=bool(int(args.stage) >= 1),
        )
    else:
        ...
        if not visual_qa_data and not image_descriptions:
            ...
            print("示例：python -m vq_lord3.data.data_collector --collect_data")
            return
    ...
    def build_train_dataloader(stage_id: int):
        use_bucket = (
            ...
            and train_bucket_meta is not None
            and not (stage_id == 2 and bool(args.disable_bucket_for_stage2))
        )
        ...
        if use_bucket:
            ...
            if stage_id == 2 and args.stage2_bucket_batch_size > 0:
                bucket_batch_size = int(args.stage2_bucket_batch_size)
    ...
    # 根据阶段训练
    if args.stage >= 0:
        stage0_dataloader = build_train_dataloader(stage_id=0)
        log_dataloader_batch_stats("stage0_loader", stage0_dataloader, tb_writer)
        if args.reuse_vq_codebook and os.path.exists(args.vq_codebook_path):
            print(f"检测到VQ codebook，跳过Stage0并加载: {args.vq_codebook_path}")
            load_vq_codebook(model, args.vq_codebook_path)
        else:
            model = train_stage0_vq(model, stage0_dataloader, args, tb_writer)
            save_stage0_checkpoint(model, args)
    
    if args.stage >= 1:
        stage1_dataloader = build_train_dataloader(stage_id=1)
        log_dataloader_batch_stats("stage1_loader", stage1_dataloader, tb_writer)
        # Stage1 复用逻辑
        if not args.stage1_ckpt_path:
            args.stage1_ckpt_path = os.path.join(args.save_path, "stage1_vision")
        
        if args.reuse_stage1 and os.path.exists(args.stage1_ckpt_path) and os.path.exists(os.path.join(args.stage1_ckpt_path, "vq_codebook.pt")):
            print(f"检测到Stage1检查点，跳过Stage1并加载: {args.stage1_ckpt_path}")
            model = load_stage1_checkpoint(model, args.stage1_ckpt_path)
        else:
            model = train_stage1_vision(model, stage1_dataloader, args, tb_writer)
            save_stage1_checkpoint(model, args, args.stage1_ckpt_path)
            save_checkpoint(model, args, "stage1_vision")
    
    if args.stage >= 2:
        if not args.stage1_ckpt_path:
            args.stage1_ckpt_path = os.path.join(args.save_path, "stage1_vision")
        if not os.path.isdir(args.stage1_ckpt_path):
            raise RuntimeError(f"Stage2 需要可用的 Stage1 checkpoint 目录: {args.stage1_ckpt_path}")
        missing_stage1_files = _stage1_contract_missing_files(args.stage1_ckpt_path)
        if missing_stage1_files:
            raise RuntimeError(
                "Stage2 启动前 Stage1 工件不完整，缺失: "
                + ", ".join(missing_stage1_files)
            )
        model = train_stage2_lord(model, train_dataset, args, tb_writer, train_bucket_meta=train_bucket_meta)
        save_stage2_checkpoint(model, os.path.join(args.save_path, "stage2_lord_final"))
