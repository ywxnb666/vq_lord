...
SOURCE_TRAIN_NUM="${SOURCE_TRAIN_NUM:-0}"
TRIM_SAMPLE_COUNT="${TRIM_SAMPLE_COUNT:-640}"
TARGET_TRAIN_NUM="${TARGET_TRAIN_NUM:-${TRIM_SAMPLE_COUNT}}"
VICTIM_TAG="$(echo "${VICTIM_MODEL}" | sed 's/[^a-zA-Z0-9._-]/_/g')"
INPUT_TEACHER_CACHE_PATH="${INPUT_TEACHER_CACHE_PATH:-${DATA_DIR}/scienceqa_teacher_${VICTIM_TAG}_${SCIENCEQA_SPLIT}_n${SOURCE_TRAIN_NUM}_seed${SCIENCEQA_SEED}_new.json}"
OUTPUT_TEACHER_CACHE_PATH="${OUTPUT_TEACHER_CACHE_PATH:-${DATA_DIR}/scienceqa_teacher_${VICTIM_TAG}_${SCIENCEQA_SPLIT}_n${TARGET_TRAIN_NUM}_seed${SCIENCEQA_SEED}_new.json}"
