...
align_vq_setup_logging "test_vq_lord_stage1_parallel"
...
STAGE1_CKPT_PATH="/home/songxinhao/workspace/vq_lord/vq_lord_ckpts/stu-qwen/a-okvqa/gemini-2.5-pro/stage1/stage1_vision_epoch1"
...
SHARD_RESULT_DIR="${SHARD_RESULT_DIR:-${TEST_RESULT_DIR}/stage1_${EVAL_SPLIT}_${EVAL_ANSWER_MODE}_vq${USE_VQ}_bucketed_shards}"
RESULT_PATH="${RESULT_PATH:-${TEST_RESULT_DIR}/stage1_${EVAL_SPLIT}_${EVAL_ANSWER_MODE}_vq${USE_VQ}_bucketed_parallel.json}"

align_vq_print_header "Stage1 产物并行评测"
...
for (( shard_id=0; shard_id<NUM_SHARDS; shard_id++ )); do
    ...
    shard_log_path="${LOG_DIR}/test_vq_lord_stage1_parallel_shard$(printf '%02d' "${shard_id}")_$(date -u +%Y%m%d_%H%M%S).log"
...
align_vq_require_file "${RESULT_PATH}" "Stage1 并行 test 结果"
eval "$(align_vq_extract_eval_metrics "${RESULT_PATH}")"

align_vq_print_header "Stage1 产物并行评测完成"
