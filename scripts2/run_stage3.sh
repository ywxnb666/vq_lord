...
SAVE_PATH="${SAVE_PATH:-${CKPT_DIR}/stage3}"
STAGE2_CKPT_PATH="${STAGE2_CKPT_PATH:-${CKPT_DIR}/stage2/stage2_vision_epoch1}"
STAGE1_CODEBOOK_PATH="${STAGE1_CODEBOOK_PATH:-${STAGE2_CKPT_PATH}/vq_codebook.pt}"
...
LR="${LR:-2e-5}"
STAGE3_LR_SCALE="${STAGE3_LR_SCALE:-0.1}"
STAGE3_PROJECTOR_LR_SCALE="${STAGE3_PROJECTOR_LR_SCALE:-0.1}"
STAGE3_GRAD_CLIP="${STAGE3_GRAD_CLIP:-1.0}"
STAGE3_TRAIN_PROJECTOR="${STAGE3_TRAIN_PROJECTOR:-1}"
# TAU1="${TAU1:-0.001}"
...
STAGE3_EVAL_MAX_NEW_TOKENS="${STAGE3_EVAL_MAX_NEW_TOKENS:-512}"
...
# Stage3 objective
STAGE3_EVAL_MAX_SAMPLES="${STAGE3_EVAL_MAX_SAMPLES:-200}"
STAGE3_EVAL_EVERY_PERIOD="${STAGE3_EVAL_EVERY_PERIOD:-1}"
if [ "${DATASET_NAME}" = "iconqa" ]; then
    STAGE3_EVAL_SPLIT="${STAGE3_EVAL_SPLIT:-test}"
else
...
STAGE3_EVAL_ANSWER_MODE="${STAGE3_EVAL_ANSWER_MODE:-generate_readable}"
STAGE3_FIELD_WEIGHT_OBSERVED="${STAGE3_FIELD_WEIGHT_OBSERVED:-1.0}"
STAGE3_FIELD_WEIGHT_CONTEXT="${STAGE3_FIELD_WEIGHT_CONTEXT:-1.0}"
STAGE3_FIELD_WEIGHT_REASONING="${STAGE3_FIELD_WEIGHT_REASONING:-2.0}"
STAGE3_FIELD_WEIGHT_ANSWER="${STAGE3_FIELD_WEIGHT_ANSWER:-12.0}"
if [ "${STUDENT_MODEL_TYPE}" = "llava_next" ]; then
    STAGE3_MC_WEIGHT="${STAGE3_MC_WEIGHT:-0.05}"
    STAGE3_LLAVA_REMOVE_CONTEXT="${STAGE3_LLAVA_REMOVE_CONTEXT:-0}"
else
    STAGE3_MC_WEIGHT="${STAGE3_MC_WEIGHT:-0.00}"
    STAGE3_LLAVA_REMOVE_CONTEXT="${STAGE3_LLAVA_REMOVE_CONTEXT:-0}"
fi
# STAGE3_OBJ_WEIGHT="${STAGE3_OBJ_WEIGHT:-0.10}"
# STAGE3_REG_WEIGHT="${STAGE3_REG_WEIGHT:-0.30}"
# STAGE3_READABLE_SFT_WEIGHT="${STAGE3_READABLE_SFT_WEIGHT:-1.00}"
# STAGE3_ANSWER_ANCHOR_WEIGHT="${STAGE3_ANSWER_ANCHOR_WEIGHT:-1.00}"
# STAGE3_ANSWER_PREFIX_WEIGHT="${STAGE3_ANSWER_PREFIX_WEIGHT:-1.00}"

STAGE3_MC_WEIGHT="${STAGE3_MC_WEIGHT:-0.00}"
STAGE3_OBJ_WEIGHT="${STAGE3_OBJ_WEIGHT:-0.10}"
STAGE3_REG_WEIGHT="${STAGE3_REG_WEIGHT:-0.30}"
STAGE3_READABLE_SFT_WEIGHT="${STAGE3_READABLE_SFT_WEIGHT:-1.0}"
STAGE3_ANSWER_ANCHOR_WEIGHT="${STAGE3_ANSWER_ANCHOR_WEIGHT:-1.0}"
STAGE3_ANSWER_PREFIX_WEIGHT="${STAGE3_ANSWER_PREFIX_WEIGHT:-1.0}"

STAGE3_PAIR_USE_ANSWER_CORRECTNESS="${STAGE3_PAIR_USE_ANSWER_CORRECTNESS:-0}"
...
STAGE3_FORCE_COLD_START_PERIOD0="${STAGE3_FORCE_COLD_START_PERIOD0:-1}"
...
STAGE3_SAMPLE_CACHE_MODEL_BASENAME="$(basename "${MODEL_PATH}")"
STAGE3_SAMPLE_CACHE_MODEL_NAME="${STAGE3_SAMPLE_CACHE_MODEL_NAME:-$(printf "%s" "${STAGE3_SAMPLE_CACHE_MODEL_BASENAME}" | tr -c 'A-Za-z0-9_.-' '_')}"
STAGE3_SAMPLE_CACHE_MODEL_PREFIX="${STAGE3_SAMPLE_CACHE_MODEL_PREFIX:-${STAGE3_SAMPLE_CACHE_MODEL_NAME}_}"
STAGE3_READABLE_SCHEMA_SUFFIX="fourfield"
if [ "${STUDENT_MODEL_TYPE}" = "llava_next" ] && [ "${STAGE3_LLAVA_REMOVE_CONTEXT}" = "1" ]; then
    STAGE3_READABLE_SCHEMA_SUFFIX="noctx"
fi
STAGE3_SAMPLE_CACHE_PATH="${STAGE3_SAMPLE_CACHE_PATH:-${DATA_DIR}/stage3/stage3_sample_cache_${STAGE3_SAMPLE_CACHE_DATASET_PREFIX}${STAGE3_SAMPLE_CACHE_MODEL_PREFIX}${VICTIM_MODEL}_${DATASET_SPLIT}_n${TRAIN_NUM}_seed${DATASET_SEED}_ml${MAX_LENGTH}_schema${STAGE3_READABLE_SCHEMA_SUFFIX}_ctx${STAGE3_VIC_INCLUDE_CONTEXT}_obs${TEACHER_OBSERVED_MAX_TOKENS}_txt${TEACHER_CONTEXT_MAX_TOKENS}_rsn${TEACHER_REASONING_MAX_TOKENS}_ans${TEACHER_ANSWER_MAX_TOKENS}.pt}"
...
STAGE2_FIELD_WEIGHT_REASONING="${STAGE2_FIELD_WEIGHT_REASONING:-1.2}"
STAGE2_FIELD_WEIGHT_ANSWER="${STAGE2_FIELD_WEIGHT_ANSWER:-2.0}"
...
STAGE3_RESULT_SCHEMA_SUFFIX=""
if [ "${STUDENT_MODEL_TYPE}" = "llava_next" ] && [ "${STAGE3_LLAVA_REMOVE_CONTEXT}" = "1" ]; then
    STAGE3_RESULT_SCHEMA_SUFFIX="_noctx"
fi
STAGE3_VALIDATION_RESULT_PATH="${STAGE3_VALIDATION_RESULT_PATH:-${TEST_RESULT_DIR}/stage3_validation_generate_readable${STAGE3_RESULT_SCHEMA_SUFFIX}.json}"
STAGE3_TEST_RESULT_PATH="${STAGE3_TEST_RESULT_PATH:-${TEST_RESULT_DIR}/stage3_test_generate_readable${STAGE3_RESULT_SCHEMA_SUFFIX}.json}"
...
echo "STAGE3_READABLE_SFT/STAGE3_OBJ_WEIGHT/STAGE3_REG_WEIGHT: ${STAGE3_READABLE_SFT_WEIGHT}/${STAGE3_OBJ_WEIGHT}/${STAGE3_REG_WEIGHT}"
...
"${TRAIN_LAUNCHER[@]}" "${TRAIN_ENTRY}" \
    ...
    --stage3_projector_lr_scale="${STAGE3_PROJECTOR_LR_SCALE}" \
    ...
    --stage3_eval_max_new_tokens="${STAGE3_EVAL_MAX_NEW_TOKENS}" \
    ...
    --stage3_readable_sft_weight="${STAGE3_READABLE_SFT_WEIGHT}" \
    --stage3_answer_anchor_weight="${STAGE3_ANSWER_ANCHOR_WEIGHT}" \
    --stage3_answer_prefix_weight="${STAGE3_ANSWER_PREFIX_WEIGHT}" \
