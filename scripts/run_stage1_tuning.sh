...
if [ "${USE_H200_PATHS}" = "1" ]; then
    ...
    export batch_size=8
    export grad_accum=8
    export stage1_lr=7e-5
    export vq_usage_decay=0.995
    export vq_dead_code_reset_interval=20
...
export bucket_batch_size="${bucket_batch_size:-4}"       # Stage1/2 分桶 batch size
export stage3_bucket_batch_size="${stage3_bucket_batch_size:-4}"  # Stage3 分桶 batch size，先保守设为 2，可稳定后再提到 4
